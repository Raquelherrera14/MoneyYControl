...
public interface PrestamoRepository extends JpaRepository<Prestamo,Long> {
    ...
    @Query("select new com.upc.tp_moneycontrol.dto.ListarPrestamosUsuarioDTO(u.idUsuario, u.nombre, u.rol.nombreRol, COUNT(p), SUM(p.monto)) " +
            "FROM Prestamo p " +
            "JOIN p.usuario u " +
            "GROUP BY u.idUsuario, u.nombre, u.rol.nombreRol")
    public List<ListarPrestamosUsuarioDTO> listarPrestamosUsuarioPorRol();


    @Query("select new com.upc.tp_moneycontrol.dto.PagoCorrespondienteDTO(t.idPrestamo,t.monto" +
    ...
    @Query("select new com.upc.tp_moneycontrol.dto.PrestamoPeriodoDTO(u.idUsuario, u.nombre, p.idPrestamo, p.monto, p.fechaInicio, " +
    ...
    @Query("select new com.upc.tp_moneycontrol.dto.EstadisticasPrestamoDTO(AVG(p.monto), SUM(p.monto), COUNT(p)) FROM Prestamo p")
